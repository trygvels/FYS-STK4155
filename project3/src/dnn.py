...
def forward(conf, X_batch, params, is_training, stride=None, pad_size=None):
    """One forward step.
    ...
    L = len(n) - 1
    batch_size = X_batch.shape[1]  # X_batch = (3,6)

    features = {}
    A = X_batch
    features["A_0"] = A
    for l in range(1, L + 1):  # For each layer in network
        A_prev = A.copy()
        Z = (
            np.dot(params["W_" + str(l)].T, A_prev) + params["b_" + str(l)]
        )  # Activation
...
# def backward(output_layer_gradient, input_layer, weight, bias, pad_size=1):
def backward(conf, Y_proposed, Y_reference, params, features, pad_size=None):
    ...
    for l in reversed(range(1, L + 1)):
        grad_params["grad_W_" + str(l)] = (
            np.dot(features["A_" + str(l - 1)], dZ.T) / m
        )  #
        grad_params["grad_b_" + str(l)] = (
        ...
        if l > 1:
            ...
            wdZ = np.sum(
                (params["W_" + str(l)].T)[:, :, np.newaxis] * dZ[:, np.newaxis, :],
                axis=0,
            )
            dZ = gZ * wdZ
            # print("wdZ {}, W_{}.T[:,:,np.newaxis] {}, dZ[:,np.newaxis,:] {}".format(wdZ.shape, l ,params['W_'+str(l)].T[:,:,np.newaxis].shape, dZ[:,np.newaxis,:].shape))
            # print("gZ {}, new dZ {}".format(gZ.shape, dZ.shape))
