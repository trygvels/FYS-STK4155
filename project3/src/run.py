...
def get_batch_indices(indices, start_index, end_index):
    ...
    n = len(indices)
    return np.hstack(
        (indices[start_index : min(n, end_index)], indices[0 : max(end_index - n, 0)])
    )


def train(conf, X_train, Y_train, X_devel, Y_devel):
    model = __import__(conf["network_type"].lower())
    ...
    # Preparation
    num_examples_in_epoch = (
        X_train.shape[1] if conf["network_type"] == "DNN" else X_train.shape[0]
    )
    example_indices = np.arange(0, num_examples_in_epoch)
    np.random.shuffle(example_indices)

    # Initialisation
    params = functions.initialization(conf)
    stride = conf["stride"]
    ...
    while True:
        ...
        if step % conf["train_progress"] == 0:
            ...
            examples_per_sec = (
                conf["batch_size"] * conf["train_progress"] / elapsed_time
            )
            ccr = num_correct / conf["batch_size"]
            running_ccr = (
                num_correct_since_last_check
                / conf["train_progress"]
                / conf["batch_size"]
            )
            num_correct_since_last_check = 0
...
def evaluate(conf, params, X_data, Y_data):
    ...
    while True:
        X_batch = (
            X_data[:, start_ind:end_ind]
            if conf["network_type"] == "DNN"
            else X_data[start_ind:end_ind, :, :, :]
        )  # The order was swapped, remember? :)
        Y_batch = functions.one_hot(Y_data[start_ind:end_ind], conf["output_dimension"])
        Y_proposal, _ = model.forward(
            conf,
            X_batch,
            params,
            stride=conf["stride"],
