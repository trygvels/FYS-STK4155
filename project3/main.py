...
def config():
    ...
    if conf["network_type"] == "CNN":
        conf["hidden_dimensions"] = [23454657687980]  # Not used
        conf["batch_size"] = 128
        conf["num_filters"] = 5
        conf["height_w"] = 3
        conf["width_w"] = 3
...
def plot_progress(train_progress, devel_progress, out_filename=None):
    ...
    ax1.plot(
        ...
        train_progress["ccr"],
        color="C0",
        label="Training set ccr",
    )
    ax1.plot(
        devel_progress["steps"],
        devel_progress["ccr"],
        color="C1",
        label="Development set ccr",
    )
...
def get_data(conf):
    ...
    if conf["dataset"] == "cifar10":
        ...
        conf["input_dimension"] = (
            conf["channels_x"] * conf["height_x"] * conf["width_x"]
        )
